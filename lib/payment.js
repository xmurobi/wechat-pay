...
Payment.prototype._signedQuery = function(url, params, options, callback, customExt, logFn){
  var self = this;
  ...
  var data = this.buildXml(params);
  if (logFn && typeof logFn == 'function') logFn(data);
  request(url, data, function(err, body){
    if(err){
      return callback(err);
    }
...
Payment.prototype.transfers = function(params, callback, logfn) {
  params = this._extendWithDefault(params, [
    'mch_appid','mchid','nonce_str'
  ]);

  this._signedQuery(URLS.TRANSFERS, params, {
    required:['partner_trade_no','openid','check_name','amount','desc','spbill_create_ip'],
    https: true
  }, callback, true, logfn);
};

Payment.prototype.getTransferInfo = function(params, callback) {
  this._signedQuery(URL.GET_TRANSFER_INFO, params, {
